...
#' )
#' }
getFullUcdpGedData <- function(date = Sys.Date(), token, candidate.only = FALSE,
                               add.metadata = TRUE) {
  ...
  ged.ls <- lapply(seq_len(nrow(ged.version.df)), function(x) {
    query.df <- getUcdpData(
      dataset = ged.version.df$dataset[x],
